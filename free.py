...
async def attack(update: Update, context: CallbackContext):
    ...
    await context.bot.send_message(chat_id=chat_id, text=(
        ...
        f"*🎯 𝙏𝙖𝙧𝙜𝙚𝙩: {ip}:{port}*\n"
        f"*🕒 �𝙙𝙪𝙧𝙖𝙩𝙞𝙤𝙣: {duration} seconds*\n"
        f"*🔥 𝙀𝙉𝙅𝙊𝙔 𝘼𝙉𝘿 𝙁𝙐𝘾𝙆 𝙒𝙃𝙊𝙇𝙀 𝙇𝙊𝘽𝘽𝙔 💥*\n"
        f"*✅𝙅𝙊𝙄𝙉 :- 𝗠𝗔𝗙𝗜𝗔𝗗𝗗𝗢𝗦*"
    ), parse_mode='Markdown')

    asyncio.create_task(run_attack(chat_id, ip, port, duration, context))  # Line 104 - Fixed (no extra parenthesis)
